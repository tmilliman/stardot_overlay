...
from stardot_overlay import get_exposure
from stardot_overlay import get_header_contents
...
if __name__ == "__main__":
    ...
    for inpath in inlist:
        ...
        try:
            exposure = int(get_exposure(inpath))
        except Exception:
            sys.stderr.write("Error extracting exposure " +
                             "for {}\n".format(inpath))
            header_lines = get_header_contents(inpath)
            print(header_lines)
            continue
